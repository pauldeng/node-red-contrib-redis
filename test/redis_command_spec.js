...
describe("redis-command node", () => {
  ...
  it("should SET hello=world and GET hello returning world", { timeout: 5000 }, function (t, done) {
    var flow = [
  ...
  it(
    "should read redis-config options from an environment variable",
    { timeout: 5000 },
    function (t, done) {
      var flow = [
        {
  ...
  it(
    "routes a redis-command SET to its own config's DB even when both configs share the default name 'Local'",
    { timeout: 5000 },
    async function () {
  ...
  describe("payload/argument handling", function () {
    ...
    [
      ...
      it(
        "SET falls back to configured Params when payload is " + c.label,
        { timeout: 5000 },
        async function () {
    ...
    scalarParamsCases.forEach(function (c) {
      it("SET applies static " + c.label + " as the value", { timeout: 5000 }, async function () {
    ...
    it(
      "null Params contributes no static argument (not an ioredis-stringified empty string)",
      { timeout: 5000 },
      async function () {
    ...
    it(
      "calls node.error (not a silent bare command) when configured Params is malformed JSON",
      { timeout: 5000 },
      async function () {
    ...
    [
      ...
      it(
        "HSET (" + c.label + ") applies the field/value object transform",
        { timeout: 5000 },
        async function () {
    ...
    [
      ...
      it(
        "MSET (" + c.label + ") applies the field/value object transform",
        { timeout: 5000 },
        async function () {
