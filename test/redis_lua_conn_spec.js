...
describe("redis-lua-script connection isolation", () => {
  ...
  it(
    "routes a non-blocking lua node to its own config's DB (not a shared pooled client)",
    { timeout: 8000 },
    async function () {
      const flow = [
  ...
  it(
    "routes a non-blocking lua node to its own config's DB even when both configs share the default name 'Local'",
    { timeout: 8000 },
    async function () {
      const flow = [
