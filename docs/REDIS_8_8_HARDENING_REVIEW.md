...
| 9   | MemoryDB and Node.js 24 verification  | **Partial** | Node.js verified: `v24.5.0`, full Docker matrix green. MemoryDB: `test/memorydb_deployment_spec.js` **7 passing** (2026-07-26) against a live cluster with `MEMORYDB_ENABLED=1`. Playwright MemoryDB editor coverage still skips unless the same credentials are present in that runner                                          |
